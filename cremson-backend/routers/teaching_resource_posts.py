...
def get_categories():
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM teaching_resource_categories ORDER BY name ASC")
...
def create_category(category: CategoryCreate):
    ...
    try:
        ...
        cursor.execute("INSERT INTO teaching_resource_categories (name) VALUES (?)", (name_clean,))
        conn.commit()
        cat_id = cursor.lastrowid
        cursor.execute("SELECT * FROM teaching_resource_categories WHERE id = ?", (cat_id,))
...
def delete_category(id_or_name: str, reassign_to: Optional[str] = None):
    ...
    if id_or_name.isdigit():
        cursor.execute("SELECT * FROM teaching_resource_categories WHERE id = ?", (int(id_or_name),))
    else:
        cursor.execute("SELECT * FROM teaching_resource_categories WHERE name = ?", (id_or_name,))
    row = cursor.fetchone()
    ...
    cursor.execute("SELECT COUNT(*) FROM teaching_resource_posts WHERE category = ?", (cat_name,))
    ...
    if count > 0:
        if reassign_to is None:
            ...
            return {
                ...
                "blog_count": count,
                "message": f"Category '{cat_name}' is assigned to {count} teaching resource post(s)."
            }
        else:
            reassign_val = "" if reassign_to in ["None", "none", ""] else reassign_to
            cursor.execute("UPDATE teaching_resource_posts SET category = ? WHERE category = ?", (reassign_val, cat_name))
            conn.commit()
            
    cursor.execute("DELETE FROM teaching_resource_categories WHERE id = ?", (cat_id,))
    conn.commit()
    conn.close()
...
def get_posts(category: Optional[str] = None, status: Optional[str] = None):
    conn = get_db_connection()
    cursor = conn.cursor()
    query = "SELECT * FROM teaching_resource_posts"
    params = []
...
def get_post(slug_or_id: str):
    ...
    if slug_or_id.isdigit():
        cursor.execute("SELECT * FROM teaching_resource_posts WHERE id = ?", (int(slug_or_id),))
    else:
        cursor.execute("SELECT * FROM teaching_resource_posts WHERE slug = ?", (slug_or_id,))
        
    row = cursor.fetchone()
    conn.close()
    
    if not row:
        raise HTTPException(status_code=404, detail="Teaching resource post not found")
...
def create_post(post: PostCreate):
    ...
    try:
        ...
        cursor.execute("""
            INSERT INTO teaching_resource_posts (slug, title, category, image, author, date, description, content, status, pdf_url, pdf_name)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (
            post.slug,
            post.title,
        ...
        cursor.execute("SELECT * FROM teaching_resource_posts WHERE id = ?", (post_id,))
        new_row = cursor.fetchone()
        conn.close()
        return dict(new_row)
    except sqlite3.IntegrityError:
...
def update_post(id: int, post: PostCreate):
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM teaching_resource_posts WHERE id = ?", (id,))
    ...
    try:
        cursor.execute("""
            UPDATE teaching_resource_posts
...
def delete_post(id: int):
    ...
    cursor.execute("DELETE FROM teaching_resource_posts WHERE id = ?", (id,))
    conn.commit()
    conn.close()
...
def toggle_post_status(id: int):
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT status FROM teaching_resource_posts WHERE id = ?", (id,))
    row = cursor.fetchone()
    if not row:
        conn.close()
        raise HTTPException(status_code=404, detail="Post not found")
    ...
    cursor.execute("UPDATE teaching_resource_posts SET status = ? WHERE id = ?", (new_status, id))
    conn.commit()
