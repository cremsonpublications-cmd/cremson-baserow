            ...
            <div className="border border-slate-200 rounded-2xl p-6 bg-slate-50 space-y-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500">Generate a single shared payment link for students to pay their share</p>
                </div>
              </div>

              {Number(order.split_count || 0) === 0 ? (
                <div className="flex flex-col sm:flex-row items-center gap-4 bg-white p-4 rounded-xl border border-slate-200">
                  ...
                  >
                    {splitLoading ? "Generating..." : "Generate Shared Link"}
                  </button>
                </div>
              ) : (
                /* Split Progress & Shared Link */
                <div className="space-y-6 bg-white p-6 rounded-xl border border-slate-200">
                  ...
                  <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 space-y-3">
                    <p className="text-xs font-bold text-slate-700 uppercase">Share this single link with all students:</p>
                    {(() => {
                      const sharedLink = typeof window !== "undefined" ? `${window.location.origin}/bulk-order/${token}/pay` : "";
                      const shareMsg = `Hi! Please use this link to pay your share of ₹${(Number(order.final_amount) / totalStudents).toFixed(2)} for our books at ${order.school_name}:\n${sharedLink}`;
                      return (
                        <div className="flex flex-col sm:flex-row gap-3">
                          <input
                            ...
                            readOnly
                            value={sharedLink}
                            className="flex-1 px-3 py-2 text-xs border border-slate-300 rounded-lg bg-white select-all font-mono"
                          />
                          <div className="flex gap-2">
                            <button
                              onClick={() => copyToClipboard(sharedLink, "shared_link")}
                              className="px-3 py-2 bg-slate-200 hover:bg-slate-300 text-slate-800 font-bold text-xs rounded-lg flex items-center gap-1 cursor-pointer"
                            >
                              <Copy className="w-3.5 h-3.5" />
                              {copiedLink === "shared_link" ? "Copied!" : "Copy"}
                            </button>
                            <a
                              href={`https://wa.me/?text=${encodeURIComponent(shareMsg)}`}
                              target="_blank"
                              rel="noreferrer"
                              className="px-3 py-2 bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs rounded-lg flex items-center gap-1 cursor-pointer"
                            >
                  ...
                  <div className="space-y-3 pt-2">
                    <p className="text-xs font-bold text-slate-700 uppercase">Received Payments:</p>
                    {students.filter(st => st.paid).length === 0 ? (
                      <p className="text-xs text-slate-400 italic">No payments received yet. Share the link above to start collecting payments.</p>
                    ) : (
                      <div className="divide-y divide-slate-100 border border-slate-200 rounded-xl max-h-72 overflow-y-auto">
                        {students.filter(st => st.paid).map((st, i) => (
                          <div key={i} className="p-3 flex items-center justify-between text-xs gap-3">
                            <div className="flex items-center gap-2">
                              <span className="w-5 h-5 bg-emerald-100 text-emerald-700 font-bold rounded-full flex items-center justify-center text-[10px]">
                                {i + 1}
                              </span>
                              <div>
