...
export default function StudyMaterialPostsPage() {
  ...
  const fetchPosts = async () => {
    try {
      setLoading(true);
      const res = await api.get("/api/study-material-posts/");
      setPosts(res.data);
    } catch (err) {
      toast.error("Failed to load study material pages.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const togglePublish = async (id) => {
    try {
      const res = await api.put(`/api/study-material-posts/${id}/status`);
  ...
  const copyLink = (slug) => {
    ...
    const fullLink = `${origin}/study-material/${slug}`;
  ...
  const confirmDelete = async () => {
    ...
    try {
      await api.delete(`/api/study-material-posts/${deleteTarget.id}`);
  ...
  return (
    <div className="flex-1 p-6 md:p-8 bg-blue-50/50 min-h-screen pb-10 text-gray-700">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-bold text-gray-800">Study Material Pages</h1>
          <p className="text-xs text-gray-400 mt-1">
            Create pages here, then copy their link and paste it as a redirect URL in the Study Materials navigation tree.
          </p>
        </div>
        <a
          href="/admin/study-material-posts/add"
              ...
              {posts.map((post, idx) => (
                <tr key={post.id} className="border-b border-gray-100 hover:bg-gray-50/50 transition-colors">
                  ...
                  <td className="px-6 py-4 font-medium text-gray-800 max-w-md truncate">
                    {post.title}
                    <div className="text-[10px] text-gray-400 font-mono mt-0.5">/study-material/{post.slug}</div>
                  </td>
                  ...
                  <td className="px-6 py-4 flex items-center gap-2 flex-wrap">
                    ...
                    <a
                      href={`/admin/study-material-posts/add?edit=${post.id}`}
                      className="border border-gray-300 px-3 py-1 text-xs rounded-lg hover:border-primary hover:text-primary transition-all cursor-pointer bg-white text-gray-600 font-medium flex items-center gap-1"
                    >
                      <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              ...
              {posts.length === 0 && (
                <tr>
                  <td colSpan="5" className="px-6 py-10 text-center text-gray-400">
                    No study material pages yet. Click &quot;Add New Page&quot; to create your first one.
                  </td>
                </tr>
              )}
            </tbody>
      ...
      {deleteTarget && (
        <ConfirmModal
          title="Delete Study Material Page"
