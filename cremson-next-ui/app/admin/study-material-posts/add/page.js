...
export default function AddStudyMaterialPostPage() {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const res = await api.get("/api/study-material-posts/categories");
  ...
  useEffect(() => {
    ...
    if (editParam) {
      ...
      const fetchDetails = async () => {
        try {
          const res = await api.get(`/api/study-material-posts/${editParam}`);
          const post = res.data;
          setTitle(post.title);
  ...
  const handleCreateCategory = async (e) => {
    ...
    setAddingCat(true);
    try {
      const res = await api.post("/api/study-material-posts/categories", { name: cleanName });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editId) {
        await api.put(`/api/study-material-posts/${editId}`, postData);
        toast.success("Page updated successfully!");
        setTimeout(() => { window.location.href = "/admin/study-material-posts"; }, 1000);
      } else {
        await api.post("/api/study-material-posts/", postData);
      ...
      <form onSubmit={handleSubmit} className="p-4 md:p-10">
        <div className="bg-white w-full max-w-7xl mx-auto p-6 md:p-10 shadow-sm border border-gray-150 rounded-xl">
          <div className="flex items-center justify-between mb-6">
            <h1 className="text-xl font-bold text-gray-800">{editId ? "Edit Study Material Page" : "Add New Study Material Page"}</h1>
            <a href="/admin/study-material-posts" className="text-xs text-gray-500 hover:text-primary transition flex items-center gap-1">
              ← Back to list
            </a>
          </div>
            ...
            >
              {uploadingImage ? (
                ...
                <div className="text-center p-4">
                  ...
                  <p className="mt-1 text-xs text-gray-500">Click to upload banner image</p>
                </div>
              )}
            </div>
          </div>
